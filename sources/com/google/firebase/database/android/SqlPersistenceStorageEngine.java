...
public class SqlPersistenceStorageEngine implements PersistenceStorageEngine {
    ...
    static {
        ...
        UTF8_CHARSET = Charset.forName(HTTP.UTF_8);
    }
    ...
    private void updateServerCache(Path path, Node node, boolean merge) {
        ...
        long duration = System.currentTimeMillis() - start;
        if (this.logger.logsDebug()) {
            this.logger.debug(String.format("Persisted a total of %d rows and deleted %d rows for a set at %s in %dms", Integer.valueOf(savedRows), Integer.valueOf(removedRows), path.toString(), Long.valueOf(duration)), new Object[0]);
        }
    }
