...
class FloatingActionButtonImplLollipop extends FloatingActionButtonImpl {
    ...
    void onElevationsChanged(float elevation, float hoveredFocusedTranslationZ, float pressedTranslationZ) {
        if (Build.VERSION.SDK_INT == 21) {
            ...
            animators.add(ObjectAnimator.ofFloat(this.view, Key.ELEVATION, elevation).setDuration(0L));
    ...
    private Animator createElevationAnimator(float elevation, float translationZ) {
        AnimatorSet set = new AnimatorSet();
        set.play(ObjectAnimator.ofFloat(this.view, Key.ELEVATION, elevation).setDuration(0L)).with(ObjectAnimator.ofFloat(this.view, View.TRANSLATION_Z, translationZ).setDuration(100L));
        set.setInterpolator(ELEVATION_ANIM_INTERPOLATOR);
        return set;
    }
